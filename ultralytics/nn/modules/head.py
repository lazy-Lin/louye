...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), mask_ratio=4, channels=(), scales=4, small_obj_weight=0.7):
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        """Initialize the YOLO Segment head with given parameters."""
        super().__init__(nc, ch)
        self.nm = nm  # number of masks
        self.npr = npr  # number of protos
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = Detect(nc, ch)  # detection head
    ...
    def forward(self, x):
        ...
        if self.training:
            return det_out, mc, p
        return (torch.cat([det_out, mc], 1), p) if self.export else (torch.cat([det_out[0], mc], 1), (det_out[1], mc, p))
